...
@Component({ 
    selector: 'KhamBenh_DaLieu',
    templateUrl: 'KhamBenh_DaLieu.component.html' 
})
export class KhamBenh_DaLieu implements OnInit {
    ...
    async ngOnInit() {
        ...
        await this.nhanVienSerivce.ListNhanVien().toPromise().then(
            ...
            error => {
                this.toastr.error(error, 'Không lấy được danh sách nhân viên');
            });
    }

    
    doCommand(command: number) {
        console.log(`KhamBenh_DaLieu đã nhận được lệnh ${command}`);
        switch (+command) {
            case Command.Save:
                this.save()
                break;
