...
@Component({
    selector: 'HanhChinh_ThanNhanTao',
    templateUrl: 'HanhChinh_ThanNhanTao.component.html'
})
export class HanhChinh_ThanNhanTao implements OnInit {
    ...
    constructor(
        ...
        //     command => {
        //         this.sharedService.confirmCommand(`HanhChinh_ThanNhanTao Recived ${command}`);
        //     });
    }
    ...
    doCommand(command: number) {
        console.log(`HanhChinh_ThanNhanTao đã nhận được lệnh ${command}`);
        switch (+command) {
            case Command.Save:
                this.save()
                break;
