...
{
    public class PhieuDanhGiaDinhDuongTreEmTbl : ThongTinKy
    {
    ...
    public class PhieuDanhGiaDinhDuongTreEmFunc
    {
        public const string TableName = "PhieuDanhGiaDinhDuongTreEm";
        public const string TablePrimaryKeyName = "IDPhieu";
        public static List<PhieuDanhGiaDinhDuongTreEmTbl> GetListData(MDB.MDBConnection MyConnection, decimal MaQuanly)
        {
            try
            {
                {
                    List<PhieuDanhGiaDinhDuongTreEmTbl> lstObject = new List<PhieuDanhGiaDinhDuongTreEmTbl>();
                    string sql = "SELECT * FROM PhieuDanhGiaDinhDuongTreEm   WHERE MaQuanLy = :MaQuanLy ORDER BY IDPhieu DESC";
                    MDB.MDBCommand command = new MDB.MDBCommand(sql, MyConnection);
                    command.Parameters.Add(new MDB.MDBParameter("MaQuanly", MaQuanly));
                    MDB.MDBDataReader DataReader = command.ExecuteReader();
                    while (DataReader.Read())
                    {
                        PhieuDanhGiaDinhDuongTreEmTbl obj = new PhieuDanhGiaDinhDuongTreEmTbl();
        ...
        public static DataSet GetDataSet(MDB.MDBConnection MyConnection, decimal IDBienBan)
        {
            string sql = @"SELECT P.*, T.SoNhapVien,
                  ...
                  FROM PhieuDanhGiaDinhDuongTreEm P 
        ...
        public static bool InsertOrUpdate(MDB.MDBConnection oracleConnection, PhieuDanhGiaDinhDuongTreEmTbl obj, ref bool isUpdate)
        {
            try
            {
                string sql = @"SELECT COUNT(*) RECNUM FROM PhieuDanhGiaDinhDuongTreEm WHERE IDPhieu  = :IDPhieu";
                ...
                {
                    isUpdate = true;
                    sql = "update PhieuDanhGiaDinhDuongTreEm set MaQuanLy=:MaQuanLy,TenBenhNhan=:TenBenhNhan,MaBenhNhan=:MaBenhNhan,Tuoi=:Tuoi,MaBenhAn=:MaBenhAn,Giuong=:Giuong,Khoa=:Khoa,Buong=:Buong,Makhoa=:Makhoa,CanNang=:CanNang,ChieuCao=:ChieuCao,BMI=:BMI,CanNangRV=:CanNangRV,MaBSDT=:MaBSDT,TenBSDT=:TenBSDT,MaNguoiTH=:MaNguoiTH,TenNguoiTH=:TenNguoiTH,DuongNuoi=:DuongNuoi,TaiKham=:TaiKham,DGBMI=:DGBMI,DGMUAC=:DGMUAC,ND1=:ND1,ND2=:ND2,ND3=:ND3,ND4=:ND4,ND5=:ND5,ND6=:ND6,ND7=:ND7,NgayTao=:NgayTao,NguoiTao=:NguoiTao,NgaySua=:NgaySua,NguoiSua=:NguoiSua";
                    sql = sql + " WHERE IDPhieu  = " + obj.IDPhieu.ToString();
                }
                else
                {
                    sql = @"insert into PhieuDanhGiaDinhDuongTreEm(MaQuanLy,TenBenhNhan,MaBenhNhan,Tuoi,MaBenhAn,Giuong,Khoa,Buong,Makhoa,CanNang,ChieuCao,BMI,CanNangRV,MaBSDT,TenBSDT,MaNguoiTH,TenNguoiTH,DuongNuoi,TaiKham,DGBMI,DGMUAC,ND1,ND2,ND3,ND4,ND5,ND6,ND7,NgayTao,NguoiTao,NgaySua,NguoiSua" +
                        ")";
                    sql = sql + @"Values(:MaQuanLy,:TenBenhNhan,:MaBenhNhan,:Tuoi,:MaBenhAn,:Giuong,:Khoa,:Buong,:Makhoa,:CanNang,:ChieuCao,:BMI,:CanNangRV,:MaBSDT,:TenBSDT,:MaNguoiTH,:TenNguoiTH,:DuongNuoi,:TaiKham,:DGBMI,:DGMUAC,:ND1,:ND2,:ND3,:ND4,:ND5,:ND6,:ND7,:NgayTao,:NguoiTao,:NgaySua,:NguoiSua" +
        ...
        public static bool DeleteByIDPhieu(MDB.MDBConnection oracleConnection, List<decimal> lstIDbienBan)
        {
            try
            {
                ...
                sql = @"DELETE FROM PhieuDanhGiaDinhDuongTreEm WHERE  " + strWhere;
