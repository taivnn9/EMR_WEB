...
{
        public class GiayCamDoanChapNhapPTTTGMHS : ThongTinKy
    {
        ...
        public string NguoiDuocDieuTri { get; set; }
        ...
        public GiayCamDoanChapNhapPTTTGMHS()
        {
            ID = 0;
            TenFileKy = "";
            UserNameKy = "";
    ...
    public class GiayCamDoanChapNhapPTTTGMHSFunc
    {

        public const string TableName = "GiayCamDoanCNPTTTGMHS";
        ...
        public static bool Delete(MDB.MDBConnection MyConnection, decimal id)
        {
            try
            {
                string sql = "DELETE FROM GiayCamDoanCNPTTTGMHS WHERE ID = :ID";
        ...
        {
            string sql = @"select * "
                         + " from GiayCamDoanCNPTTTGMHS WHERE ID =: ID ORDER BY NgayTao, NgaySua asc";
        ...
        public static bool InsertOrUpdate(MDB.MDBConnection MyConnection, GiayCamDoanChapNhapPTTTGMHS ketQua)
        {
            try
            {
                string sql = @"INSERT INTO GiayCamDoanCNPTTTGMHS
                (
                    MaQuanLy,
                    DanToc,
                    NgoaiKieu,
                ...
                if (ketQua.ID != 0)
                {
                    sql = @"UPDATE GiayCamDoanCNPTTTGMHS SET 
                    MaQuanLy = :MaQuanLy,
                    DanToc = :DanToc,
                    NgoaiKieu = :NgoaiKieu,
                    ...
                    DiaChi = :DiaChi,
                    NguoiThan = :NguoiThan,
                    NguoiDuocDieuTri = :NguoiDuocDieuTri,
                ...
                Command.Parameters.Add(new MDB.MDBParameter("NguoiDuocDieuTri", ketQua.NguoiDuocDieuTri));
        ...
        public static List<GiayCamDoanChapNhapPTTTGMHS> GetData(MDB.MDBConnection MyConnection, decimal MaQuanLy)
        {
            List<GiayCamDoanChapNhapPTTTGMHS> lstData = new List<GiayCamDoanChapNhapPTTTGMHS>();
            string sql = @"select * from GiayCamDoanCNPTTTGMHS WHERE MaQuanLy = :MaQuanLy ";
            ...
            using (Command = new MDB.MDBCommand(sql, MyConnection))
            {
                ...
                {
                    GiayCamDoanChapNhapPTTTGMHS item = new GiayCamDoanChapNhapPTTTGMHS();

                    item.ID = long.Parse(DataReader["ID"].ToString());

                    item.MaBenhNhan = XemBenhAn._HanhChinhBenhNhan.MaBenhNhan;
                    item.HoTen = XemBenhAn._HanhChinhBenhNhan.TenBenhNhan;
                    item.Tuoi = XemBenhAn._HanhChinhBenhNhan.Tuoi;
                    item.GioiTinh = XemBenhAn._HanhChinhBenhNhan.GioiTinh.Description();
                    item.DiaChi = Common.GetDiaChi();
                    item.SoYTe = XemBenhAn._HanhChinhBenhNhan.SoYTe;
                    item.BenhVien = XemBenhAn._HanhChinhBenhNhan.BenhVien;
                    ...
                    item.NguoiDuocDieuTri = ConDBNull(DataReader["NguoiDuocDieuTri"]);
