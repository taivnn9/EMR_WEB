...
{
    public class ThongBaoDTCovidThanhCong : ThongTinKy
    {
        public ThongBaoDTCovidThanhCong()
        {
            TableName = "ThongBaoDTCovidThanhCong";
            TablePrimaryKeyName = "ID";
            IDMauPhieu = (int)DanhMucPhieu.TBDTCTC;
            TenMauPhieu = DanhMucPhieu.TBDTCTC.Description();
        }
    ...
    public class ThongBaoDTCovidThanhCongFunc
    {
        public const string TableName = "ThongBaoDTCovidThanhCong";
        public const string TablePrimaryKeyName = "ID";
        public static List<ThongBaoDTCovidThanhCong> GetListData(MDB.MDBConnection _OracleConnection, decimal maquanly)
        {
            List<ThongBaoDTCovidThanhCong> list = new List<ThongBaoDTCovidThanhCong>();
            try
            {
                string sql = @"SELECT * FROM ThongBaoDTCovidThanhCong where MaQuanLy = :MaQuanLy";
                MDB.MDBCommand Command = new MDB.MDBCommand(sql, _OracleConnection);
                Command.Parameters.Add(new MDB.MDBParameter("MaQuanLy", maquanly));
                MDB.MDBDataReader DataReader = Command.ExecuteReader();
                while (DataReader.Read())
                {
                    ThongBaoDTCovidThanhCong data = new ThongBaoDTCovidThanhCong();
        ...
        public static bool InsertOrUpdate(MDB.MDBConnection MyConnection, ref ThongBaoDTCovidThanhCong ketQua)
        {
            try
            {
                string sql = @"INSERT INTO ThongBaoDTCovidThanhCong
                (
                    MAQUANLY,
                    MaBenhNhan,
                    So,
                ...
                if (ketQua.ID != 0)
                {
                    sql = @"UPDATE ThongBaoDTCovidThanhCong SET 
                    MAQUANLY = :MAQUANLY,
        ...
        public static bool delete(MDB.MDBConnection MyConnection, long id)
        {
            try
            {
                string sql = "DELETE FROM ThongBaoDTCovidThanhCong WHERE ID = :ID";
                MDB.MDBCommand command = new MDB.MDBCommand(sql, MyConnection);
