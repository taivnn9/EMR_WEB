...
{
    public class TaiKhamBAStentDMV : ThongTinKy
    {
        public TaiKhamBAStentDMV()
        {
            TableName = "TaiKhamBAStentDMV";
    ...
    public class TaiKhamBAStentDMVFunc
    {
        public const string TableName = "TaiKhamBAStentDMV";
        public const string TablePrimaryKeyName = "ID";
        public static List<TaiKhamBAStentDMV> GetListData(MDB.MDBConnection _OracleConnection, decimal maquanly)
        {
            List<TaiKhamBAStentDMV> list = new List<TaiKhamBAStentDMV>();
            try
            {
                string sql = @"SELECT * FROM TaiKhamBAStentDMV where MaQuanLy = :MaQuanLy";
                MDB.MDBCommand Command = new MDB.MDBCommand(sql, _OracleConnection);
                Command.Parameters.Add(new MDB.MDBParameter("MaQuanLy", maquanly));
                MDB.MDBDataReader DataReader = Command.ExecuteReader();
                while (DataReader.Read())
                {
                    TaiKhamBAStentDMV data = new TaiKhamBAStentDMV();
        ...
        public static bool InsertOrUpdate(MDB.MDBConnection MyConnection, ref TaiKhamBAStentDMV ketQua)
        {
            try
            {
                string sql = @"INSERT INTO TaiKhamBAStentDMV
                (
                    MAQUANLY,
                    MaBenhNhan,
                    TaiKhamLan,
                ...
                if (ketQua.ID != 0)
                {
                    sql = @"UPDATE TaiKhamBAStentDMV SET 
                    MAQUANLY = :MAQUANLY,
        ...
        public static bool delete(MDB.MDBConnection MyConnection, long id)
        {
            try
            {
                string sql = "DELETE FROM TaiKhamBAStentDMV WHERE ID = :ID";
                MDB.MDBCommand command = new MDB.MDBCommand(sql, MyConnection);
