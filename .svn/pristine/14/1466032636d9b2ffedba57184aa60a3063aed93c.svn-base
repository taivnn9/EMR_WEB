...
{
    public class BBHCTPT : ThongTinKy
    {
        public BBHCTPT()
        {
            TableName = "BBHCTPT";
            TablePrimaryKeyName = "ID";
            IDMauPhieu = (int)DanhMucPhieu.BBHCTPT;
            TenMauPhieu = DanhMucPhieu.BBHCTPT.Description();
        }
        ...
        [MoTaDuLieu("Sinh thiết")]
        public string TagSinhThiet { get; set; }
        [MoTaDuLieu("Xét nghiệm khác")]
        ...
        [MoTaDuLieu("Điện thoại liên lạc")]
    ...
    public class BBHCTPTFunc
    {
        public const string TableName = "BBHCTPT";
        public const string TablePrimaryKeyName = "ID";
        public static List<BBHCTPT> GetListData(MDB.MDBConnection _OracleConnection, decimal maquanly)
        {
            List<BBHCTPT> list = new List<BBHCTPT>();
            try
            {

                string sql = @"SELECT * FROM BBHCTPT where MaQuanLy = :MaQuanLy";
                ...
                while (DataReader.Read())
                {
                    try
                    {
                        ...
                        data.SinhThiet = Common.ConDBNull(DataReader["SinhThiet"]);
        ...
        public static bool InsertOrUpdate(MDB.MDBConnection _OracleConnection, ref BBHCTPT obj)
        {
            string sql;
            int n = 0;
            {
                ...
                if (obj.TagSinhThiet.IsNotNullOrEmpty())//32
                {
                    MDB.MDBCommand CommandIDxetnghiem = new MDB.MDBCommand(sqlIdXetnghiem, _OracleConnection);
                    CommandIDxetnghiem.Parameters.Add(new MDB.MDBParameter("ID", (int)(EMR_MAIN.CLS_PTTT.SinhThiet)));
                    MDB.MDBDataReader DataReaderIdxetnghiem = CommandIDxetnghiem.ExecuteReader();
                    rowCountId = 0;
                    while (DataReaderIdxetnghiem.Read()) rowCountId++;
                    if (rowCountId == 0)
                    {
                        sql = @"INSERT INTO MapIdxetnghiem(ID, IDXETNGHIEM, IDLOAI) VALUES(" + (int)(EMR_MAIN.CLS_PTTT.SinhThiet) + "," + obj.TagSinhThiet.ToInt32() + ", 0)";
                        CommandIDxetnghiem = new MDB.MDBCommand(sql, _OracleConnection);
                        nId = CommandIDxetnghiem.ExecuteNonQuery();
                    }
                }
            ...
            oracleCommand.Parameters.Add(new MDB.MDBParameter("SinhThiet", obj.SinhThiet));
        ...
        {
            string sql = "DELETE FROM BBHCTPT WHERE ID = :ID";
