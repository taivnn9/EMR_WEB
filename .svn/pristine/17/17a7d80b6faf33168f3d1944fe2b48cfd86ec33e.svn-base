...
@Component({ 
    selector: 'KhamBenh_BenhAnSanKhoa',
    templateUrl: 'KhamBenh_BenhAnSanKhoa.component.html' 
})
export class KhamBenh_BenhAnSanKhoa implements OnInit {
    ...
    doCommand(command: number) {
        console.log(`KhamBenh_BenhAnSanKhoa đã nhận được lệnh ${command}`);
        switch (+command) {
            case Command.Save:
                this.save()
                break;
