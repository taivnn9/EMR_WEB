...
namespace EMR_MAIN.DATABASE.Other
{
    ...
    public class PhieuTheoDoiVaChamSocNgoaiKhoa : ThongTinKy
    {
        public PhieuTheoDoiVaChamSocNgoaiKhoa()
        {
            TableName = "PhieuTDVCSBNNgoaiKhoa";
            TablePrimaryKeyName = "ID";
            IDMauPhieu = (int)DanhMucPhieu.PTDVCSNGOAIKHOA;
            TenMauPhieu = DanhMucPhieu.PTDVCSNGOAIKHOA.Description();
        }
        ...
        public List<PhieuTheoDoiVaChamSocNgoaiKhoa_ChiTiet> TTCSChiTiet { get; set; }
        ...
        public PhieuTheoDoiVaChamSocNgoaiKhoa Clone()
        {
            PhieuTheoDoiVaChamSocNgoaiKhoa other = (PhieuTheoDoiVaChamSocNgoaiKhoa)this.MemberwiseClone();
            other.TTCSChiTiet = new List<PhieuTheoDoiVaChamSocNgoaiKhoa_ChiTiet>();
            if (this.TTCSChiTiet != null)
            foreach (PhieuTheoDoiVaChamSocNgoaiKhoa_ChiTiet item in this.TTCSChiTiet)
            {
        ...
        public string TinhTrangVanDong { get; set; }
        public string DinhDuong { get; set; }

        public bool[] TrungTien_Array { get; } = new bool[] { false, false};
        public string TrungTien
        {
            get
            {
                string temp = String.Empty;
                for (int i = 0; i < TrungTien_Array.Length; i++)
                    temp += (TrungTien_Array[i] ? "1" : "0");
                return temp;
            }
            set
            {
                if (value != null)
                {
                    ...
                    {
                        int intTemp = value[i] - '0';
                        TrungTien_Array[i] = intTemp == 1 ? true : false;
                    }
                }
            }
        }
        public bool[] VetThuong_Array { get; } = new bool[] { false, false, false, false,false };
    ...
    public class PhieuTheoDoiVaChamSocNgoaiKhoa_ChiTiet: ThongTinKy
    {
        [MoTaDuLieu("Mã định danh")]
        ...
        [MoTaDuLieu("Chọn", null, 0, 0)]
		public bool Chon { get; set; }
        public PhieuTheoDoiVaChamSocNgoaiKhoa_ChiTiet Clone()
        {
            PhieuTheoDoiVaChamSocNgoaiKhoa_ChiTiet other = (PhieuTheoDoiVaChamSocNgoaiKhoa_ChiTiet)this.MemberwiseClone(); 
            return other;
        }
    }
    public class PhieuTheoDoiVaChamSocNgoaiKhoaFunc
    {
        static Logger log = LogManager.GetLogger("Log");
        public const string TableName = "PhieuTDVCSBNNgoaiKhoa";
        public const string TablePrimaryKeyName = "IDPhieu";

        public static List<ThuocPha> DanhSachThuocPha = new List<ThuocPha>();


        public List<PhieuTheoDoiVaChamSocNgoaiKhoa> Select(MDB.MDBConnection MyConnection, string MaQuanLy)
        {
            try
            { 
                { 
                    List<PhieuTheoDoiVaChamSocNgoaiKhoa> listResult = new List<PhieuTheoDoiVaChamSocNgoaiKhoa>();
                    string sql = @"SELECT t.*
                    FROM PhieuTDVCSBNNgoaiKhoa t
                    ...
                    {
                        var obj = new PhieuTheoDoiVaChamSocNgoaiKhoa();
                        ...
                        obj.TinhTrangVanDong = Common.ConDBNull(DataReader["TinhTrangVanDong"]);
                        obj.DinhDuong = Common.ConDBNull(DataReader["DinhDuong"]);
                        obj.TrungTien = Common.ConDBNull(DataReader["TrungTien"]);
                        ...
                        obj.TTCSChiTiet = new List<PhieuTheoDoiVaChamSocNgoaiKhoa_ChiTiet>();
        ...
        public bool Insert(MDB.MDBConnection MyConnection, PhieuTheoDoiVaChamSocNgoaiKhoa obj)
        {
            try
            { 
                { 
                    string sql = @"INSERT INTO PhieuTDVCSBNNgoaiKhoa (
                    MaQuanLy,
                    TenKhoa,
                    MaKhoa,
                    MaBenhNhan,
                    ...
                    oracleCommand.Parameters.Add(new MDB.MDBParameter("TinhTrangVanDong", obj.TinhTrangVanDong));
                    oracleCommand.Parameters.Add(new MDB.MDBParameter("DinhDuong", obj.DinhDuong));
                    oracleCommand.Parameters.Add(new MDB.MDBParameter("TrungTien", obj.TrungTien));
        ...
        public bool Update(MDB.MDBConnection MyConnection, PhieuTheoDoiVaChamSocNgoaiKhoa obj)
        {
            try
            {
                {

                    string sql = @"UPDATE PhieuTDVCSBNNgoaiKhoa SET 
                        MaQuanLy=:MaQuanLy,
                        TenKhoa=:TenKhoa,
                        MaKhoa=:MaKhoa,
                        ...
                        TinhTrangVanDong = :TinhTrangVanDong,
                        DinhDuong = :DinhDuong,
                        TrungTien = :TrungTien,
                        VetThuong = :VetThuong,
                        Dau = :Dau,
        ...
        public bool Delete(MDB.MDBConnection MyConnection, decimal IDPhieu)
        {
            try
            {
                if (IDPhieu != 0)
                {
                    string sql = @"Delete PhieuTDVCSBNNgoaiKhoa 
                                WHERE 
                                (IDPhieu = :IDPhieu) ";
                    MDB.MDBCommand Command = new MDB.MDBCommand(sql, MyConnection);
                    ...
                    Command.Dispose();
                    sql = @"Delete PTDVCSBNNgoaiKhoa_ChiTiet 
                                WHERE 
                                (IDPhieu = :IDPhieu) ";
        ...
        public bool Delete_ChiTiet(MDB.MDBConnection MyConnection, decimal IDPhieuCT)
        {
            try
            {
                if (IDPhieuCT != 0)
                {
                    string sql = @"Delete PTDVCSBNNgoaiKhoa_ChiTiet 
                                WHERE 
                                (IDPhieuCT = :IDPhieuCT) ";
                    MDB.MDBCommand Command = new MDB.MDBCommand(sql, MyConnection);
        ...
        public static List<PhieuTheoDoiVaChamSocNgoaiKhoa_ChiTiet> Select_PhieuCHiTiet(MDB.MDBConnection MyConnection, string IDPhieu, int ModOrder = 0)
        {
            try
            {  
                {
                    List<PhieuTheoDoiVaChamSocNgoaiKhoa_ChiTiet> listResult = new List<PhieuTheoDoiVaChamSocNgoaiKhoa_ChiTiet>();
                    string sql = @"SELECT t.*
                    FROM PTDVCSBNNgoaiKhoa_ChiTiet t
                    ...
                    while (DataReader.Read())
                    {
                        var obj = new PhieuTheoDoiVaChamSocNgoaiKhoa_ChiTiet();
        ...
        public bool InsertOrUpdate_ChiTiet(MDB.MDBConnection MyConnection, PhieuTheoDoiVaChamSocNgoaiKhoa_ChiTiet obj)
        {
            try
            {
                string sql = @"update PTDVCSBNNgoaiKhoa_ChiTiet set
                    IDPhieu=:IDPhieu,
                    ThoiGian=:ThoiGian,
                    NguoiTao=:NguoiTao,
                ...
                if (n == 0)
                {
                    ...
                    sql = @"insert into PTDVCSBNNgoaiKhoa_ChiTiet (
                    IDPhieu,
                    ThoiGian,
                    NguoiTao,
                    MaNguoiTao,
