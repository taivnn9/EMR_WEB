...
namespace EMR_MAIN.DATABASE.Other
{
    ...
    public class PTDVCSNBTN : ThongTinKy
    {
        public PTDVCSNBTN()
        {
            TableName = "PTDVCSNBTN";
            TablePrimaryKeyName = "ID";
            IDMauPhieu = (int)DanhMucPhieu.PTDVCSNBTN;
            TenMauPhieu = DanhMucPhieu.PTDVCSNBTN.Description();
        }
        ...
        public List<PTDVCSNBTN_ChiTiet> TTCSChiTiet { get; set; }
        ...
        public PTDVCSNBTN Clone()
        {
            PTDVCSNBTN other = (PTDVCSNBTN)this.MemberwiseClone();
            other.TTCSChiTiet = new List<PTDVCSNBTN_ChiTiet>();
            if (this.TTCSChiTiet != null)
            foreach (PTDVCSNBTN_ChiTiet item in this.TTCSChiTiet)
            {
    ...
    public class PTDVCSNBTN_ChiTiet: ThongTinKy
    {
        [MoTaDuLieu("Mã định danh")]
        ...
        [MoTaDuLieu("Chọn", null, 0, 0)]
		public bool Chon { get; set; }
        public PTDVCSNBTN_ChiTiet Clone()
        {
            PTDVCSNBTN_ChiTiet other = (PTDVCSNBTN_ChiTiet)this.MemberwiseClone(); 
            return other;
        }
    }
    public class PTDVCSNBTNFunc
    {
        static Logger log = LogManager.GetLogger("Log");
        public const string TableName = "PTDVCSNBTN";
        public const string TablePrimaryKeyName = "IDPhieu";

        public static List<ThuocPha> DanhSachThuocPha = new List<ThuocPha>();


        public List<PTDVCSNBTN> Select(MDB.MDBConnection MyConnection, string MaQuanLy)
        {
            try
            { 
                { 
                    List<PTDVCSNBTN> listResult = new List<PTDVCSNBTN>();
                    string sql = @"SELECT t.*
                    FROM PTDVCSNBTN t
                    ...
                    while (DataReader.Read())
                    {
                        ...
                        obj.TTCSChiTiet = new List<PTDVCSNBTN_ChiTiet>();
        ...
        public bool Insert(MDB.MDBConnection MyConnection, PTDVCSNBTN obj)
        {
            try
            { 
                { 
                    string sql = @"INSERT INTO PTDVCSNBTN (
                    MaQuanLy,
                    TenKhoa,
                    MaKhoa,
                    MaBenhNhan,
        ...
        public bool Update(MDB.MDBConnection MyConnection, PTDVCSNBTN obj)
        {
            try
            {
                {

                    string sql = @"UPDATE PTDVCSNBTN SET 
                        MaQuanLy=:MaQuanLy,
                        TenKhoa=:TenKhoa,
                        MaKhoa=:MaKhoa,
        ...
        public bool Delete_ChiTiet(MDB.MDBConnection MyConnection, decimal IDPhieuCT)
        {
            try
            {
                if (IDPhieuCT != 0)
                {
                    string sql = @"Delete PTDVCSNBTN_ChiTiet 
                                WHERE 
                                (IDPhieuCT = :IDPhieuCT) ";
                    MDB.MDBCommand Command = new MDB.MDBCommand(sql, MyConnection);
        ...
        public static List<PTDVCSNBTN_ChiTiet> Select_PhieuCHiTiet(MDB.MDBConnection MyConnection, string IDPhieu, int ModOrder = 0)
        {
            try
            {  
                {
                    List<PTDVCSNBTN_ChiTiet> listResult = new List<PTDVCSNBTN_ChiTiet>();
                    string sql = @"SELECT t.*
                    FROM PTDVCSNBTN_ChiTiet t
                    ...
                    while (DataReader.Read())
                    {
                        var obj = new PTDVCSNBTN_ChiTiet();
        ...
        public bool InsertOrUpdate_ChiTiet(MDB.MDBConnection MyConnection, PTDVCSNBTN_ChiTiet obj)
        {
            try
            {
                string sql = @"update PTDVCSNBTN_ChiTiet set
                    IDPhieu=:IDPhieu,
                    ThoiGian=:ThoiGian,
                    NguoiTao=:NguoiTao,
                ...
                if (n == 0)
                {
                    ...
                    sql = @"insert into PTDVCSNBTN_ChiTiet (
                    IDPhieu,
                    ThoiGian,
                    NguoiTao,
                    MaNguoiTao,
