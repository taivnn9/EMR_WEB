...
{
    public class ThuThuatDatCatheterTMCT : ThongTinKy
    {
        public ThuThuatDatCatheterTMCT()
        {
            TableName = "ThuThuatDatCatheterTMCT";
            TablePrimaryKeyName = "ID";
            IDMauPhieu = (int)DanhMucPhieu.PTTTDCTMCT;
            TenMauPhieu = DanhMucPhieu.PTTTDCTMCT.Description();
        }
    ...
    public class ThuThuatDatCatheterTMCTFunc
    {
        public const string TableName = "ThuThuatDatCatheterTMCT";
        public const string TablePrimaryKeyName = "ID";

        public static List<ThuThuatDatCatheterTMCT> GetListData(MDB.MDBConnection _OracleConnection, decimal maquanly)
        {
            List<ThuThuatDatCatheterTMCT> list = new List<ThuThuatDatCatheterTMCT>();
            try
            {
                string sql = @"SELECT * FROM ThuThuatDatCatheterTMCT where MaQuanLy = :MaQuanLy";
                MDB.MDBCommand Command = new MDB.MDBCommand(sql, _OracleConnection);
                Command.Parameters.Add(new MDB.MDBParameter("MaQuanLy", maquanly));
                MDB.MDBDataReader DataReader = Command.ExecuteReader();
                while (DataReader.Read())
                {
                    ThuThuatDatCatheterTMCT data = new ThuThuatDatCatheterTMCT();
        ...
        public static bool DeleteByIDPhieu(MDB.MDBConnection oracleConnection, decimal IDBienBan)
        {
            try
            {
                ...
                string sql = @"DELETE FROM ThuThuatDatCatheterTMCT WHERE ID =" + IDBienBan;
        ...
        public static DataSet GetDataSet(MDB.MDBConnection MyConnection, long IDBienBan)
        {
            string sql = @"SELECT P.*, T.SoNhapVien, T.Khoa,  T.MaKhoa, T.MaBenhAn, H.BENHVIEN, H.SoYTe,  H.TenBenhNhan, H.Tuoi
			  FROM ThuThuatDatCatheterTMCT P 
        ...
        public static bool InsertOrUpdate(MDB.MDBConnection MyConnection, ref ThuThuatDatCatheterTMCT ketQua)
        {
            try
            {
                string sql = @"INSERT INTO ThuThuatDatCatheterTMCT
                (
                    MaQuanLy,
                    Buong,
                    Giuong,
                ...
                if (ketQua.ID != 0)
                {
                    sql = @"UPDATE ThuThuatDatCatheterTMCT SET 
                    MaQuanLy = :MaQuanLy,
                    Buong = :Buong,
                    Giuong = :Giuong,
