...
{
        public class KhaoSatDienSinhLyCDVDT : ThongTinKy
    {
        public KhaoSatDienSinhLyCDVDT()
        {
            TableName = "KhaoSatDienSinhLyCDVDT";
            TablePrimaryKeyName = "ID";
            IDMauPhieu = (int)DanhMucPhieu.GCKDYDTBLRLN;
            TenMauPhieu = DanhMucPhieu.GCKDYDTBLRLN.Description();
        }
    ...
    public class KhaoSatDienSinhLyCDVDTFunc
    {

        public const string TableName = "KhaoSatDienSinhLyCDVDT";
        ...
        public static bool Delete(MDB.MDBConnection MyConnection, decimal id)
        {
            try
            {
                string sql = "DELETE FROM KhaoSatDienSinhLyCDVDT WHERE ID = :ID";
        ...
        public static List<KhaoSatDienSinhLyCDVDT> GetListData(MDB.MDBConnection _OracleConnection, decimal maquanly)
        {
            List<KhaoSatDienSinhLyCDVDT> list = new List<KhaoSatDienSinhLyCDVDT>();
            try
            {
                string sql = @"SELECT * FROM KhaoSatDienSinhLyCDVDT where MaQuanLy = :MaQuanLy";
                MDB.MDBCommand Command = new MDB.MDBCommand(sql, _OracleConnection);
                Command.Parameters.Add(new MDB.MDBParameter("MaQuanLy", maquanly));
                MDB.MDBDataReader DataReader = Command.ExecuteReader();
                while (DataReader.Read())
                {
                    KhaoSatDienSinhLyCDVDT data = new KhaoSatDienSinhLyCDVDT();
        ...
        public static bool InsertOrUpdate(MDB.MDBConnection MyConnection, KhaoSatDienSinhLyCDVDT ketQua)
        {
            try
            {
                string sql = @"INSERT INTO KhaoSatDienSinhLyCDVDT
                (
                    MaQuanLy,
                    HoTen,
                    TinhTrangBenh,
                ...
                if (ketQua.ID != 0)
                {
                    sql = @"UPDATE KhaoSatDienSinhLyCDVDT SET 
                    MaQuanLy = :MaQuanLy,
                    HoTen = :HoTen,
